exports.SimpleHttpRequest = function(requestURI, callback) {
	var url = require('url');
	...
	httpRequest.on('response', function(response) {
		responseObject = response;
                ...
                if (response.headers && response.headers['location']) {
                  isRedirected = true;
                  httpRequest.end();
                  exports.SimpleHttpRequest(response.headers['location'], callback);
                }
		...
		responseObject.on('end', function() {
			...
			process.nextTick(function() {
				callback(responseObject, responseBuffer);
			});
                  }
		});
	});
