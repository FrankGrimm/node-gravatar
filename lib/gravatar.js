...
exports.getProfileFor = function(hash, callback) {
  ...
  SimpleHttpRequest(requestUrl, function(err, response, data) {
    // if errors occured (e.g. in a redirect), escalate it to the callback
    ...
    if (response.statusCode == 200) {
      ...
      callback(new Error('HTTP Error code: ' + response.statusCode), null);
    }
  });
}
